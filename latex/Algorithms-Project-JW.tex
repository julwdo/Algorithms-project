...
SON was applied to this dataset of baskets with a minimum support threshold of 0.01\%, as higher thresholds resulted in very few frequent itemsets. Typically, the support threshold is set much higher (e.g., 1\%), but this dataset had a particular characteristic:\ a large number of distinct books, each \textit{liked} by only a few users. This pattern could partly reflect the dataset’s nature. However, many books appeared under slightly different titles, such as \textit{The Hobbit or There and Back Again} and \textit{The Hobbit; Or, There and Back Again}. Consequently, these variations were treated as separate books, leading to distinct support counts rather than being aggregated.

The code was executed on Google Colab, a cloud-based platform providing 12.67 GB of RAM and 2 virtual CPU cores. The number of partitions was fixed at 2, as testing with 4, 8, 16, and 32 partitions led to increased execution time. Based on the number of baskets in each partition and the entire dataset, along with the chosen support threshold, the local and global minimum count thresholds were computed. The maximum itemset size \( k_{\max} \) was set to 2. The execution time was 244.04 seconds (approx.\ 4 minutes and 4 seconds), identifying 7,289 frequent singletons and 9,565 frequent pairs.
